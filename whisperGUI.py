...
def start_GUI():
    ...
    save_output_dir_setting_key = "-REMEMBER-OUTPUT-DIR-"
    ...
    def make_main_window() -> sg.Window:
        ...
        # Get user's last choice for translating to English or not
        ...
        tab1_layout = [
            ...
            [
                ...
                sg.Image(
                    checked_box_image
                    if translate_to_english_last_choice
                    else unchecked_box_image,
                    key=translate_to_english_checkbox_key,
                    metadata=translate_to_english_last_choice,
                    enable_events=True,
                ),
            ],
            [
            ...
            [
                sg.Checkbox(
                    ...
                    default=sg.user_settings_get_entry(
                        save_output_dir_setting_key, False
                    ),
                    key=save_output_dir_setting_key,
                    auto_size_text=True,
                )
            ],
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, "Exit"):
            ...
            if sg.user_settings_get_entry(save_output_dir_setting_key):
        ...
        # User clicked a checkbox
            ...
            sg.user_settings_set_entry(
                ...
                window[translate_to_english_checkbox_key].metadata,
            )
        # User saved settings
            ...
            is_saving_output_directory = values[save_output_dir_setting_key]
            sg.user_settings_set_entry(
                save_output_dir_setting_key, is_saving_output_directory
            )

            # Delete the save output directory in the settings file
...
def transcribe_audio_video_files(
    ...
    Args:
        ...
        translate_to_english (bool): True if the user has chosen to translate each transcription to English, False otherwise.
    """
    ...
    for audio_video_path in audio_video_file_paths:
        ...
        output_paths = write_transcript_to_files(
            result, audio_video_path, output_dir_path
        )
...
def write_transcript_to_files(
    ...
    """Write the results of a whisper transcription to .txt, .vtt, and .srt files with the same name as the source file.
